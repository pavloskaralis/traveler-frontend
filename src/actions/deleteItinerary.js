...
export default function deleteItinerary(itinerary, userID) {
    return dispatch => {
        ...
        const removeRequests = async () => {
            const result = await axios.delete(`http://traveler-backend.herokuapp.com/lookups/${userID}/${itinerary.id}`); 
            const {data} = result;     
            if(!data.error){
                history.push('/');
                dispatch(toggleForm(''));
        ...
        const deleteRequest = async () => {
            const result = await axios.delete(`http://traveler-backend.herokuapp.com/itineraries/${itinerary.id}`);  
            const {data} = result;  
            if(!data.error){
                history.push('/');
                dispatch(toggleForm(''));
